...
def exit_on_Ne_str_parsing_fail(Ne_str) :
    print('Failed parsing Ne parameter: {0}'.format(Ne_str), file=sys.stderr)
    print('Exiting', file=sys.stderr)
    sys.exit(1)

# prints the interpretation of Ne argument
def print_Ne_interpretation(Ne0, scheme, Ne_range) :
    print('Ne argument interpreted as:', file=sys.stderr)
    print('\tNe at founding generation: {0}'.format(Ne0), file=sys.stderr)
    print('\tNe variation scheme: {0}'.format(scheme), file=sys.stderr)
    print('\tNe final range: {0}'.format(Ne_range), file=sys.stderr)
...
def exit_on_contrib_parsing_fail(contrib_str, contrib_id) :
    print('Failed parsing contrib_s{0} parameter: {1}'.format(contrib_id, contrib_str), file=sys.stderr)
    print('Exiting', file=sys.stderr)
    sys.exit(1)
...
def parse_pulse_contrib(contrib_str, contrib_id) :
    ...
    print('Contribution {0} interpreted as:'.format(contrib_id), file=sys.stderr)
    print('\tFounding contribution: {0}'.format(contrib[0]), file=sys.stderr)
    print('\tContribution scheme: Pulse', file=sys.stderr)
    for i in range(contrib[2]) :
        print('\tPulse {0} range: {1}'.format(i+1, contrib[3+i]), file=sys.stderr)
    return contrib
...
def exit_on_contrib_inconsistency(contrib_id) :
    print('ERROR: Contribution {0} pattern is inconsistent. Exiting'.format(contrib_id), file=sys.stderr)
    sys.exit(1)
...
def make_contrib_sanity_check(contrib, contrib_id) :
    ...
    if scheme == 'Inc' :
        ...
        print('WARNING: Adjusting ranges for contribution {0}'.format(contrib_id), file=sys.stderr)
        print('\tInitial contribution range: {0}-{1}'.format(c_init_min, c_init_max), file=sys.stderr)
        print('\tFinal contribution range: {0}-{1}'.format(c_final_min, c_final_max), file=sys.stderr)
...
def exit_on_compatibility_failure() :
    print('ERROR: contributions patterns are not compatible. Exiting', file=sys.stderr)
    sys.exit(1)
...
def generate_non_constant_pattern(x1, x2, y1, y2, increase) :
    if x2 == x1 :
        print('ERROR: cannot generate non constant pattern over 1 generation.\nExiting', file=sys.stderr)
        sys.exit(1)
...
def exit_on_nb_try(max_nb_try) :
    print('Failed to generate valid contributions pattern after {0} tries. Exiting'.format(max_nb_try), file=sys.stderr)
    sys.exit(1)
...
def exit_on_nb_pulse_error(contrib_id) :
    print('ERROR: too much pulses for contribution {0}.\nExiting'.format(contrib_id), file=sys.stderr)
    sys.exit(1)
...
def main() :
    ...
    try :
        ...
        if not force_rewrite :
            print('Directory {0} already exists. Exiting'.format(prefix), file=sys.stderr)
            sys.exit(1)
    ...
    for i in range(1, nb_simulation+1) :
        ...
        try :
            ...
            pass
        f = open(filename, 'w')
        print('g\tNe\tc1\tc2', file=f)
        for j in range(nb_generation+1) :
            print('\t'.join([str(j), str(new_Nes[j]), str(contribs_s1[j]), str(contribs_s2[j])]), file=f)
        f.close()
